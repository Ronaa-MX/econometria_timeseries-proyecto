...
de tiempo de una red espacio-temporal para conocer sus condiciones respecto al
...
a considerar del modelo *ARIMA* del método Box-Jenkins, donde queremos revisar 
los parámetros:
* Autoregresión *P*
* Integración *I*
* Media Movíl *Q*
...
diferencias entre el valor actual y sus lags.
```{r}
# Diferenciación
...
consideramos un criterio de información más sencillo siendo este el "Akaike 
...
# Extra-Análisis de grafo ponderado
...
Se muestra la frecuencia de los valores ponderados de los vértices entre los 
nodos, dado que en la red se conectan los nodos con todos los de la misma red, 
se considera al grafo uno completo, los valores de uno son los de los nodos 
conectados consigo mismos. Además de uns resumen de los valores de las 
ponderaciones de los vértices de la red.
```{r}
summary(E(g)$weight)
hist(E(g)$weight, main = "Distribución de pesos", xlab = "Peso")
```
## Mapa de calor del grafo completo ponderado
Como en el caso anterior se muestra en base a un color más oscruro a claro, 
llendo de un valor entre 0 a 1 de los vértices ponderados entre los vértices.
```{r}
...
Dimensiones de la matriz de adayaciencia además de los valores de las conexiones
entre los nodos a través de la lista de vértices.
```{r}
...
Gráfica de toda la red donde se presenta los nodos y vértices, donde los pesos
se representan a través del grosor del vértice que conecta los nodos.
```{r}
# Crear grafo
